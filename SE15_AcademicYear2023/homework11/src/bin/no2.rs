...
impl Document {
    ...
    fn generate_html_para(ranked: &[Document]) -> String {
    ...
    fn generate_html_word(ranked: &[Document]) -> String {
        ...
        for doc in ranked {
            html_table.push_str("<tr>");
            html_table.push_str(&format!("<td>{}</td>", doc.name));
            html_table.push_str(&format!("<td>{:>02}</td>", doc.word_count));
            html_table.push_str("</tr>\n");
        }
        html_table.push_str("</table>\n");
        html_table
...
fn main() {
    ...
    let para_html_table = Document::generate_html_para(&ranked_by_para_count);
    // Write the HTML file
    ...
    let word_html_table = Document::generate_html_word(&ranked_by_word_count);
    // Write the HTML file
